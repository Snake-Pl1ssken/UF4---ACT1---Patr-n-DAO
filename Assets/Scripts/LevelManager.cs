...
public class LevelManager : MonoBehaviour
{
    ...
    public Transform uiRetryButton;
    ...
    void Start()
    {
        ...
        gateStats = new GateStat[gateCount];

        retryButtonC = uiRetryButton.GetComponent<UIButton>();
        retryButtonC.onPressed +=  OnRetryPressed;
    ...
    void Update()
    {
        ...
        {
            if(retryPressed) { SceneManager.LoadScene(SceneManager.GetActiveScene().buildIndex); }
        }
        ...
        if(state != nextState)
        {
            ...
            else if(nextState == State.finished)
            {
                ...
                int count = statsManagerC.GetGateStats(gateStats);

                uiStatsC.text = "";

                for(int i = 0; i < count; i++)
                {
                    uiStatsC.text += String.Format("Gate {0:0}: Time {1:0.00} Speed {2:0.00}\n", i + 1, gateStats[i].time, gateStats[i].speed);
                }
